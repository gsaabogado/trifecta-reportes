...
Two surfaces:
  1. Reportes    — branded inspection reports from a GDrive/Dropbox folder link
  2. Facturación — step-by-step guide for manual CFDI 4.0 timbrado on the
                   SAT free portal (issue + cancel), with SNCG's real values
"""
...
from sat_guide import render_sat_guide
...
def render_reports_tab() -> None:
    """The original report-generation flow."""
...
tab_reportes, tab_facturas = st.tabs(["📋 Reportes", "🧾 Facturación SAT"])

# The SAT guide renders first in code order: the reports flow uses st.stop()
# on some error paths, which would otherwise blank this tab.
with tab_facturas:
    render_sat_guide()
